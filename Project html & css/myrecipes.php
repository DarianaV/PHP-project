...
<?php  
        ...
        $sqlQuery="SELECT * FROM recipes WHERE id_user = '$userID'";
        $result = mysqli_query($link, $sqlQuery) or die(mysqli_error($link));        
        if($result) {
        while ($recipe = mysqli_fetch_array($result)) {    
        $image = $recipe['pic'];
        $recipeID = $recipe['id'];                
                echo "<div class='design'><td><a href=delete.php?id_recipe=".$recipe['id'].">DELETE</a></td>" .
                 "<td><a href=edit.php?id_recipe=".$recipe['id'].">EDIT</a></td>" .
                  '<p class="new-recipe">' . $recipe['title'] . "</p>" . "<p>" .$recipe['text'] . "</p>" .   
                '<img src="image/'.$image.'" height = "120" width="150" class="img"/>' . "</div>";             
