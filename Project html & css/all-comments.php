...
$_SESSION['id_edit'] = $recipesID;
 $sqlQuery="SELECT * FROM recipes WHERE id = '$recipesID'";
        ...
        $image = $recipe['pic'];
                echo "<div class='new'><td><a href=all-comments.php?id_recipe=".$recipe['id']."></a></td>" .
       ...
       $query = "SELECT comments.text FROM comments JOIN recipes ON recipes.id = comments.id_recipes WHERE comments.id_recipes = '$recipesID'";
       $re = mysqli_query($link, $query) or die(mysqli_error($link));       
       if($re){
       while($comment = mysqli_fetch_array($re))
       {
             $commentText = $comment['text'];
             echo "<p class='comment-field'>" . $commentText . "</p>";
       }
     }
?>
<form method="POST" action="comment.php" class="comments">
<div>
  <textarea class="field-comment" name="comment"></textarea>
  <input type="submit" name="make-comment"/>
</div>
</form>
</section>
</main>
